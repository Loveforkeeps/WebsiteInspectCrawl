...
class website():
    ...
    def __init__(self, url, retry=RETRY_TIMES):
        ...
        try:
            self.res = self.session.get(
                self.url, verify=False, headers=self.Headers, timeout=5)
            ...
            if len(self.res.content):
                ...
                if self.encode:  # 自解码
                    try:
                        ...
                        self.content = self.res.content.decode(
                            self.encode, 'ignore')
    ...
    def parser(html):
        new_emails = set(re.findall(
            r"[a-z0-9\.\-+_]+@[a-z0-9\.\-+_]+\.[a-z]+", req.text, re.I))
        new_domain = set(re.findall(
            'https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+', req.text, re.I))
...
def inspect_coinhive(domain):
    ...
    try:
        ...
        if not len(r) == 2:
            ...
            if hasattr(p, "res"):
                # print("初始化成功！")
                ret = dict(Target=domain, IP=ip,
                           Url=p.res.url, coinhive=p.checkjs())
            else:
                # print(r+"无法访问")
            ...
            try:
                ...
                for i in open(sys.argv[1], "r"):
                    ...
                    if hasattr(p, "res"):
                        ...
                        print("无法访问")
                        datas.append(dict(Target=p.url, Url=None,
